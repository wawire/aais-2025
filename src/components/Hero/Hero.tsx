...
// Updated actions with mailto link for sponsor button
const DEFAULT_ACTIONS: readonly HeroAction[] = [
  {
    id: 'sponsor',
    label: 'Become a Sponsor',
    href: 'mailto:aais@kenya-airways.com?subject=AAIS 2025 Sponsorship Inquiry&body=Hello,%0D%0A%0D%0AI am interested in becoming a sponsor for the Africa Aviation Innovation Summit 2025. Please provide me with sponsorship packages and details.%0D%0A%0D%0AThank you.',
    variant: 'primary',
    ariaLabel: 'Email AAIS team about sponsorship opportunities for AAIS 2025',
    external: true, // Mark as external to handle mailto properly
  },
  {
    id: 'register',
    ...
    // external: false (default) - internal Next.js route
  },
] as const;
 ...
 * Updated to handle mailto links for sponsor contact
 */
const Hero = memo<HeroProps>(
  ({
    ...
    // Memoized action button renderer with enhanced mailto handling
    const renderActionButton = useCallback((action: HeroAction) => {
      const baseClasses =
        'relative inline-flex items-center justify-center px-6 py-3 md:px-8 md:py-4 rounded-lg font-semibold text-base md:text-lg transition-all duration-300 transform hover:scale-105 active:scale-95 focus:outline-none focus:ring-4 focus:ring-offset-2 focus:ring-offset-transparent';

      const variantClasses = {
        primary:
          'bg-aviationGold hover:bg-yellow-600 hover:text-white text-white shadow-lg hover:shadow-aviationGold/40 focus:ring-aviationGold',
        secondary:
          'bg-white hover:bg-gray-50 text-charcoal-900 shadow-lg hover:shadow-white/20 focus:ring-white',
        outline:
          'border-2 border-white text-white hover:bg-white hover:text-charcoal-600 shadow-lg hover:shadow-white/20 focus:ring-white backdrop-blur-sm',
      };

      // Determine if it's an external link (mailto, https, etc.) or internal Next.js Link
      const isExternalLink =
        action.external || action.href.startsWith('mailto:') || action.href.startsWith('http');
      const ButtonComponent = isExternalLink ? 'a' : Link;

      // Enhanced link props for external links (especially mailto)
    ...
    const renderCountdownUnit = useCallback(
